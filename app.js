...
// 2. Hàm đổ dữ liệu lên Form để Sửa (Thủ công bằng tay)
async function editCard(id) {
    // Ép kiểu ID về dạng chuẩn (Số hoặc Chuỗi văn bản)
    const safeId = isNaN(Number(id)) ? id : Number(id);
    
    // BƯỚC CẢI TIẾN SIÊU TỐC: Thay vì gọi database, lấy trực tiếp từ hàm getAllCards() đã lưu trên RAM
    const allCards = await getAllCards();
    const card = allCards.find(c => c.id === safeId);
    ...
    // Đổ dữ liệu lên form nhập liệu ngay lập tức
    ...
    // Cuộn màn hình mượt mà lên vùng Form nhập liệu để bạn sửa
...
// =========================================================================
// ⚡ TỐI ƯU HÓA TỐC ĐỘ: HÀM XÓA TỪ CHẠY TRƠN TRU
// =========================================================================
async function deleteCard(id) {
    if (confirm("Bạn có chắc chắn muốn xóa từ này khỏi kho dữ liệu không?")) {
        ...
        await deleteCardFromDB(safeId);
        
        // Đồng bộ đám mây chạy ẩn phía sau (Không làm nghẽn giao diện)
        syncLocalToCloud(); 

        // Làm mới bộ chọn chủ đề và danh sách từ hiển thị lập tức
        if (typeof refreshDeckSelector === "function") {
            await refreshDeckSelector();
        }
        await renderWordsList();
...
async function renderWordsList(resetToPageOne = false) {
    if (resetToPageOne) {
        currentPage = 1; // Khởi động lại về trang đầu nếu người dùng gõ tìm kiếm hoặc đổi bộ lọc [cite: 33, 34]
    }

    let searchTimeout = null;
    const searchInput = document.getElementById("search-word-input");
    if (searchInput) {
    // Sử dụng sự kiện "input" để nhận biết ngay khi gõ hoặc xóa kí tự
    searchInput.addEventListener("input", () => {
        // 1. Xóa lượt chờ cũ nếu người dùng vẫn đang gõ liên tục nhằm chống đơ máy
        if (searchTimeout) {
            clearTimeout(searchTimeout);
        }

        // 2. Tạo độ trễ 300ms sau khi dừng gõ mới quét dữ liệu hiển thị
        searchTimeout = setTimeout(async () => {
            console.log("Đang quét dữ liệu hiển thị trực tiếp...");
            
            // 🔥 ĐÃ SỬA: Thêm từ khóa 'await' trước hàm renderWordsList
            // Hàm này là async function (bất đồng bộ) nên ép trình duyệt phải đợi nạp xong
            // dữ liệu từ database IndexedDB rồi mới vẽ giao diện ngay lập tức tại chỗ.
            await renderWordsList(true); 
    ...
    // 1. ÁP DỤNG BỘ LỌC CHỦ ĐỀ CỦA BẠN 
    if (typeof currentSelectedDeck !== "undefined" && currentSelectedDeck !== "all") {
        cards = cards.filter(c => c.deck === currentSelectedDeck); 
    }

    // 2. 🧠 BỘ LỌC TÌM KIẾM THÔNG MINH MỚI (Gõ h tìm h, gõ he tìm he...)
    if (keyword) {
        cards = cards.filter(card => {
            const word = (card.word || "").toLowerCase();
            
            // 🔥 THAY ĐỔI CỐT LÕI: Ép buộc từ vựng PHẢI BẮT ĐẦU bằng cụm ký tự đang gõ
            // Ví dụ: Gõ "c" -> Chỉ giữ lại "center", "cutting"... loại bỏ hoàn toàn các từ chứa chữ c ở giữa như "induction"
    ...
    // 3.⚡ LUỒNG HIỂN THỊ SIÊU TỐC KHÔNG LAG: GOM TRANG KHI ĐANG TÌM KIẾM
    let cardsToDisplay = [];
    
    if (keyword) {
        // Có từ khóa: Hiện toàn bộ kết quả trên 1 phân trang để người dùng nhìn thấy ngay
        cardsToDisplay = cards;
        if (paginationEl) paginationEl.innerHTML = ""; // Ẩn thanh phân trang cũ tránh rối mắt
    } else {
        // Không có từ khóa: Trả về logic chia trang mượt mà như cũ của bạn
        ...
        cardsToDisplay = cards.slice(startIndex, endIndex); 
    }

    // 4. HIỂN THỊ CÁC TỪ VỰNG TRÊN TRANG HIỆN TẠI (ĐÃ TỐI ƯU CỐ ĐỊNH CHỦ ĐỀ DÀI) [cite: 45]
    cardsToDisplay.forEach(card => {
        // Tối ưu tag chủ đề: cố định độ rộng tối đa, nếu dài quá tự động chuyển thành dấu 3 chấm (...) [cite: 45]
        ...
        // 🔥 ĐÃ SỬA: Thêm dấu nháy đơn '${card.id}' vào thuộc tính onclick của editCard và deleteCard để an toàn tuyệt đối với mọi loại chủ đề
        li.innerHTML = `
    ...
    if (paginationEl) {
        ...
        // Chữ hiển thị thông số trang (ví dụ: Trang 2 / 10) [cite: 59]
        ...
        // Nút tiến lên (Trang sau) [cite: 61]
